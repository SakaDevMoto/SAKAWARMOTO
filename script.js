...
function isAutoAssignedPlayerName(value) {
  return /^player(?:\s+\d+)?$/i.test(String(value ?? "").trim());
}
...
function currentProfile({ customName = true } = {}) {
  ...
  const followsAssignedAutoName =
    customName &&
    Boolean(trimmedName) &&
    (trimmedName === assignedAutoName || (!localRecord?.nameCustomized && isAutoAssignedPlayerName(trimmedName)));
...
function getPlayerModerationEntry(snapshot, player) {
  if (!snapshot || !player) {
    return { kick: null, ban: null };
  }

  const moderation = snapshot.moderation || {};
  const kick = moderation.kicks?.[player.id] || null;
  const ban = player.clientId ? moderation.bans?.[player.clientId] || null : null;
  return { kick, ban };
}

function getPlayerStateTag(snapshot, player) {
  const moderation = getPlayerModerationEntry(snapshot, player);
  if (moderation.ban?.permanent) {
    return "Banido perm.";
  }
  if (moderation.ban) {
    return "Banido";
...
function renderPlayerModerationActions(player, { compact = false } = {}) {
  ...
  const moderation = getPlayerModerationEntry(snapshot, player);
  if (!canModerate || moderation.kick || moderation.ban) {
    return "";
  }
