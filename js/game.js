...
const POINT_TARGET_ABILITIES = new Set(["tempest:E", "volt:E", "volt:R"]);
const COLOR_WHITE = new THREE.Color("#ffffff");
const REMOTE_PREDICTION_LEAD_MS = 90;
const REMOTE_PREDICTION_MAX_MS = 220;
const REMOTE_CATCHUP_RATE = 14;
const REMOTE_MAX_PREDICTION_SPEED = 420;
...
function renderAbilityIconSvg(ability) {
  switch (ability?.id) {
    ...
    case "phase-cloak":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M4 12c2.4-4 5.6-6 8-6s5.6 2 8 6c-2.4 4-5.6 6-8 6s-5.6-2-8-6z"></path>
          <path d="M12 9.3a2.7 2.7 0 1 0 0 5.4 2.7 2.7 0 0 0 0-5.4z"></path>
    ...
    case "recon-beacon":
      return `
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M12 4.2v4.4"></path>
          <path d="M12 15.4v4.4"></path>
          <circle cx="12" cy="12" r="2.6"></circle>
          <path d="M5.4 12a6.6 6.6 0 0 1 13.2 0"></path>
          <path d="M3.2 12a8.8 8.8 0 0 1 17.6 0"></path>
        </svg>
      `;
    case "pierce-line":
      return `
...
export class BattleRoyaleGame {
  ...
  isPointTargetAbility(loadoutId, slot) {
    return POINT_TARGET_ABILITIES.has(`${loadoutId}:${slot}`);
  }
  ...
  resolvePerformanceProfile() {
    ...
    return {
      pixelRatioCap: mobile ? (crowded ? 0.78 : busy ? 0.9 : 1) : crowded ? 1 : busy ? 1.15 : 1.3,
      fxScale: mobile ? (crowded ? 0.34 : busy ? 0.46 : 0.6) : crowded ? 0.46 : busy ? 0.64 : 0.84,
      maxParticles: mobile ? (crowded ? 60 : busy ? 90 : 130) : crowded ? 120 : busy ? 170 : 240,
      shadowsEnabled: !mobile && playerCount < 6,
    };
  }
  ...
  syncLocalPoseFromSnapshot() {
    ...
    const running = this.snapshot.meta?.state === "running";
    const movement = this.getMovementInputVector();
    const activelyDrivingPose =
      running &&
      localRecord.alive !== false &&
      movement.magnitude > 0.08;
    const incomingPoseSeq = Math.max(0, Number(localRecord.poseSeq) || 0);
    if (running && localRecord.alive !== false && incomingPoseSeq < this.localPoseSeq) {
      return;
    }

    const farAway = Math.hypot(localRecord.x - this.localPose.x, localRecord.y - this.localPose.y) > 120;
    if (!running || farAway || localRecord.alive === false) {
      this.localPose.x = localRecord.x;
      this.localPose.y = localRecord.y;
      this.localPose.aim = localRecord.aim;
      this.localPoseSeq = Math.max(this.localPoseSeq, incomingPoseSeq);
      return;
    }

    if (activelyDrivingPose && incomingPoseSeq <= this.localPoseSeq) {
      return;
    }
  ...
  updateRemotePlayers(players) {
    ...
    Object.entries(players).forEach(([playerId, player]) => {
      ...
      const existing = this.remotePlayers.get(playerId) || {
        ...
        targetY: player.y,
        aim: player.aim,
        targetAim: player.aim,
        velocityX: 0,
        velocityY: 0,
        targetUpdatedAt: Number(player.updatedAt) || Date.now(),
      };
      ...
      const incomingPoseSeq = Math.max(0, Number(player.poseSeq) || 0);
      if (incomingPoseSeq >= (existing.poseSeq || 0)) {
        const incomingUpdatedAt = Number(player.updatedAt) || Date.now();
        const previousTargetX = Number.isFinite(existing.targetX) ? existing.targetX : player.x;
        const previousTargetY = Number.isFinite(existing.targetY) ? existing.targetY : player.y;
        const previousUpdatedAt = Number(existing.targetUpdatedAt) || incomingUpdatedAt;
        const deltaSeconds = Math.max(0.001, (incomingUpdatedAt - previousUpdatedAt) / 1000);
        let velocityX = (player.x - previousTargetX) / deltaSeconds;
        let velocityY = (player.y - previousTargetY) / deltaSeconds;
        const velocityMagnitude = Math.hypot(velocityX, velocityY);
        if (velocityMagnitude > REMOTE_MAX_PREDICTION_SPEED) {
          const ratio = REMOTE_MAX_PREDICTION_SPEED / velocityMagnitude;
          velocityX *= ratio;
          velocityY *= ratio;
        }
        existing.targetX = player.x;
        existing.targetY = player.y;
        existing.targetAim = player.aim;
        existing.velocityX = velocityX;
        existing.velocityY = velocityY;
        existing.targetUpdatedAt = incomingUpdatedAt;
        existing.poseSeq = incomingPoseSeq;
      }
  ...
  processEvent(event) {
    switch (event.type) {
      ...
      case "projectile-impact":
        this.expireProjectileVisual(event.projectileId);
      ...
      case "shield-pop":
        this.groundEffects.push(
          this.createGroundEffectVisual({
            ...
            x: event.x,
            y: event.y,
            radius: event.radius || 76,
            color: event.color || "#9ee4ff",
            expiresAt: Date.now() + 420,
          })
        );
        this.spawnBurst(event.x, event.y, "#eef8ff", 10, 78, 24);
        this.spawnTinyDots(event.x, event.y, event.color || "#9ee4ff", 6, 20, PLAYER_Y + 12);
        break;
      case "damage": {
  ...
  expireProjectileVisual(projectileId) {
    if (!projectileId) {
      return null;
    }
  ...
  createBeamVisual(fromX, fromY, toX, toY, color, width, life) {
    ...
    const mesh = new THREE.Mesh(this.sharedGeometries.beam, material);
  ...
  expireArcOrbVisual(projectileId, x, y, color) {
    this.expireProjectileVisual(projectileId);
  ...
  update(deltaMs, now) {
    ...
    const remoteFollowFactor = 1 - Math.exp(-REMOTE_CATCHUP_RATE * delta);

    this.remotePlayers.forEach((player) => {
      const targetUpdatedAt = Number(player.targetUpdatedAt) || now;
      const predictionMs = Math.min(
        REMOTE_PREDICTION_MAX_MS,
        Math.max(0, now - targetUpdatedAt) + REMOTE_PREDICTION_LEAD_MS
      );
      const predictedX = clamp(
        (player.targetX ?? player.x) + (player.velocityX || 0) * (predictionMs / 1000),
        PLAYER_RADIUS,
        WORLD_SIZE - PLAYER_RADIUS
      );
      const predictedY = clamp(
        (player.targetY ?? player.y) + (player.velocityY || 0) * (predictionMs / 1000),
        PLAYER_RADIUS,
        WORLD_SIZE - PLAYER_RADIUS
      );
      player.x += (predictedX - player.x) * remoteFollowFactor;
      player.y += (predictedY - player.y) * remoteFollowFactor;
      const targetAim = player.targetAim ?? player.aim ?? 0;
      player.aim += wrapAngle(targetAim - (player.aim ?? 0)) * Math.min(1, remoteFollowFactor * 1.2);
    });
  ...
  updatePlayerVisuals(now, localCombatVisual = null) {
    ...
    players.forEach((player) => {
      ...
      visual.coreMaterial.color.set(playerColor).lerp(COLOR_WHITE, 0.24);
      ...
      visual.weapon.material.color.set(playerColor).lerp(COLOR_WHITE, 0.14);
